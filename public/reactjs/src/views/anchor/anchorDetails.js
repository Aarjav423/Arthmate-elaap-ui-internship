  ...
  const fetchAnchorDocs = () => {
    dispatch(
      fetchAnchorDocsWatcher(
        { anchor_id: pid },
        async docs => {
          let copyAnchorDocumentArray = JSON.parse(
            JSON.stringify(anchorDocumentArray)
          );
  ...
  return (
    <Grid container xs={12} style={{ padding: "10px 10px 0 40px" }}>
        ...
        <Grid item xs={6}>
          <Grid item xs={12}>
            ...
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>Anchor ID</Typography>
              <Typography style={fieldStyle}>{anchorData._id}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>Anchor Name</Typography>
              <Typography style={fieldStyle}>{anchorData.name}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>CIN</Typography>
              <Typography style={fieldStyle}>{anchorData.cin}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>GSTIN</Typography>
              <Typography style={fieldStyle}>{anchorData.gstin}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>Address</Typography>
              <Typography style={fieldStyle}>
                {anchorData.anchor_address}
              </Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>State</Typography>
              <Typography style={fieldStyle}>{anchorData.state}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>City</Typography>
              <Typography style={fieldStyle}>{anchorData.city}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>Pin code</Typography>
              <Typography style={fieldStyle}>{anchorData.pin_code}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>Website</Typography>
              <Typography style={fieldStyle}>{anchorData.website}</Typography>
            </Grid>
            <Grid item xs={12} style={formFieldStyle}>
              <Typography style={labelStyle}>Business Phone</Typography>
              <Typography style={fieldStyle}>
                {anchorData.business_phone}
              </Typography>
            </Grid>
          </Grid>
          <Divider />
          <Grid item xs={12}>
            ...
            {anchorData.directors.map((director, index) => {
              return (
                ...
                >
                  <Typography style={labelStyle}>{`Director ${
                    index + 1
                  }`}</Typography>
      ...
      <Grid item xs={6}>
        ...
        >
          {anchorDocumentArray.length &&
            anchorDocumentArray.map((item, index) => {
              return (
                <Grid
                  item
                  key={`${item.name}-${index}`}
