...
const {
  getCoLenderRepaymentAPI,
  getCoLenderRepaymentSummaryAPI
} = require("../../../apis/colenderReports.js");
...
const DownloadCoLendingRepaymentFile = props => {
  const {
    isCustomDatePicker,
    text,
    onSearchClick,
  ...
  const [repaymentFiles, setRepaymentFiles] = useState([]);
  ...
  const handleSearch = async value => {
    ...
    try {
      const response = await getCoLenderRepaymentAPI(payload);
      const data = response.data;
      setRepaymentFiles(response.data);
    } catch (err) {
  ...
  const handleDownloadReport = async item => {
    ...
    const response = await getCoLenderRepaymentSummaryAPI(payload);
    if (!response) {
      ...
      <CardContent>
        <Grid container>
          ...
          >
            ...
            {customDate ? (
              <Grid xs={3} sx={{ ml: "12px" }} item>
                <Button
                  disabled={
                    isTagged
                      ? !checkAccessTags([
                          "tag_co_lending_repayment_file_read_write"
                        ])
                      : false
                  }
                  style={{
            ...
            <Grid xs={12}>
              ...
              >
                ...
                >
                  <TableHead>
                    <TableRow>
                      <StyledTableCell>Repayment Summary</StyledTableCell>
                      <StyledTableCell>
                      ...
                      .map((x, index) => {
                        return (
                          <StyledTableRow key={index}>
                            <StyledTableCell>{x.file_name}</StyledTableCell>
                            <StyledTableCell>
                            ...
                            <StyledTableCell>
                              ...
                              >
                                <IconButton
                                  ...
                                  color="primary"
                                  onClick={e => handleDownloadReport(x)}
                                >
                                  <ArrowCircleDownIcon />
                                </IconButton>
                              </Tooltip>
...
export default DownloadCoLendingRepaymentFile;
